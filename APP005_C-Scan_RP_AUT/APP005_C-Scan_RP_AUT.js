...
function onOpen() {
    SpreadsheetApp.getUi()
        .createMenu('⚙️ Reportes C-Scan RP AUT')
...
function generarPorIdManual() {
    ...
    iniciarColaDeGeneracion([id]);
}
function mostrarPanelSelectorMasivo() { mostrarPanelSelector(); }
function generarTodosPendientes() {
    ...
    if (resp === ui.Button.YES) iniciarColaDeGeneracion(pendientes.map(function(p){ return p.id; }));
}
