 ...
 * ===========================================================================
 * CORRECCIÓN APLICADA:
 * - insertarImagenFlotante_MT() reemplaza newCellImage() / =IMAGE()
 ...
 * - NUEVO: las imágenes ahora se CENTRAN dentro del área de la celda/rango
 *   (offset X/Y) en lugar de quedar pegadas a la esquina superior izquierda.
 * ===========================================================================
 */
...
function generarReportePorId_MT(idBuscado) {
  ...
  // Insertar firma (imagen flotante centrada)
  ...
  // Insertar fotos (imágenes flotantes centradas)
...
function procesarFotosCombinadas_MT(hojaDest, fotos, filaInicioFotos, filaInicioDesc) {
  ...
  fotos.forEach((fotoData, index) => {
    ...
    if (index > 1 && esPar) {
      ...
      // Limpieza: solo las celdas que vamos a rellenar
    ...
    // Insertar imagen flotante centrada y descripción
    if (esPar) {
...
// =================================================================
// --- CORRECCIÓN IMÁGENES: FLOTANTES CENTRADAS EN EL ÁREA ---
// =================================================================

/**
 * Inserta una imagen FLOTANTE anclada en la celda dada y la CENTRA dentro
 * del área del rango (horizontal y verticalmente).
 *
 * Centrado:
 *   offsetX = Math.max(0, Math.round((anchoArea - anchoFinal) / 2))
 *   offsetY = Math.max(0, Math.round((altoArea  - altoFinal)  / 2))
 */
